...
export class TimeRangeMark implements IGraph, IGraphStyle {
    private _chart: any;
    private _series: any;
    ...
    private markType: MarkType = MarkType.TimeRange;
        ...
        if (!this._renderer) {
            this._renderer = {
                draw: (target: any) => {
                    ...
                    ctx.beginPath();
                    ctx.moveTo(startX, Math.min(startY, endY));
                    ctx.lineTo(startX, Math.max(startY, endY));
                    ctx.moveTo(endX, Math.min(startY, endY));
                    ctx.lineTo(endX, Math.max(startY, endY));
                    ctx.stroke();
