...
interface CCIIndicatorInfo {
  id: string;
  params: CCIIndicatorParam[];
  nonce: number;
}

interface CCISettingModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (indicator: CCIIndicatorInfo) => void;
  initialIndicator?: CCIIndicatorInfo | null;
  theme?: ThemeConfig;
  parentRef?: React.RefObject<HTMLDivElement | null>;
}

const CCISettingModal: React.FC<CCISettingModalProps> = ({
  isOpen,
  onClose,
  onConfirm,
  ...
  const [indicator, setIndicator] = useState<CCIIndicatorInfo | null>(null);
  ...
  const cciPeriods = [14, 20, 30, 40, 50];

  useEffect(() => {
    if (initialIndicator) {
      setIndicator(initialIndicator);
    } else {
      const defaultIndicator: CCIIndicatorInfo = {
  ...
  const addIndicatorParam = () => {
    ...
    const availablePeriods = cciPeriods.filter(p => !usedPeriods.includes(p));
    if (availablePeriods.length === 0) return;
    const randomColor = getRandomColor();
    const newPeriod = availablePeriods[0];
    const newParam: CCIIndicatorParam = {
      paramName: `CCI${newPeriod}`,
  ...
  const updateIndicatorValue = (paramIndex: number, value: number) => {
    ...
    const validValue = cciPeriods.includes(value) ? value : 20;
  ...
  const webkitScrollbarStyle = `
        .cci-scrollbar::-webkit-scrollbar {
            width: 6px;
        }
        .cci-scrollbar::-webkit-scrollbar-track {
            background: ${theme?.toolbar?.background || '#fafafa'};
            border-radius: 3px;
        }
        .cci-scrollbar::-webkit-scrollbar-thumb {
            background: ${theme?.toolbar?.border || '#d9d9d9'};
            border-radius: 3px;
        }
        .cci-scrollbar::-webkit-scrollbar-thumb:hover {
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <div style={modalTitleStyle}>CCI指标设置</div>
          </div>
          <div style={modalBodyStyle}>
            <div
              style={indicatorsListStyle}
              className="cci-scrollbar"
            >
              {indicator.params?.map((param, paramIndex) => {
                const availablePeriods = cciPeriods.filter(p =>
            ...
            <button
              ...
              type="button"
              title={indicator.params.length >= 5 ? "最多添加5个CCI参数" : "添加CCI参数"}
            >
              {indicator.params.length >= 5 ? "已达到最大参数数量(5个)" : "+ 添加CCI参数"}
            </button>
...
export const CCIIndicator: React.FC<CCIIndicatorProps> = ({
  theme,
  data,
  height,
  width,
  ...
  const calculateMultipleCCI = (data: ICandleViewDataPoint[]) => {
    ...
    indicatorSettings.params.forEach(param => {
      ...
      if (cciData.length > 0) {
        result[param.paramName] = cciData;
      }
    });
    return result;
  };
  ...
  const handleConfirmSettings = (newSettings: CCIIndicatorInfo) => {
      ...
      <div style={{
        ...
        <div style={{
          ...
          <button
            ...
            onClick={() => handleRemoveSubChartIndicator && handleRemoveSubChartIndicator(SubChartIndicatorType.CCI)}
            onMouseEnter={(e) => {
              e.currentTarget.style.background = theme.toolbar.button.hover;
              e.currentTarget.style.opacity = '1';
            }}
