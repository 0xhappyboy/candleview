...
export class HLCArea implements IMainChart {
    ...
    private _data: ICandleViewDataPoint[] = [];
    ...
    constructor(chartLayer: Chart, theme: ThemeConfig) {
        ...
        this.series = chartLayer.chart!.addSeries(LineSeries, {
            ...
            lineWidth: this._lineWidth as 1 | 2 | 3 | 4,
    ...
    time() {
        return this._data.length > 0 ? this._data[0].time : 0;
    }

    priceValue() {
        return this._data.length > 0 ? this._data[0].close : 0;
    }
    ...
    private drawChannel(ctx: CanvasRenderingContext2D): void {
        ...
        const visibleValidData = this._data.filter(item =>
            !item.isVirtual &&
