...
export class ElliottTriangleMark implements IGraph, IMarkStyle {
    private _chart: any;
    private _series: any;
    ...
    private markType: DrawingType = DrawingType.Elliott_Triangle;
    ...
    private drawFibonacciLevels(ctx: any, coordinates: any[]) {
        ...
        coordinates.forEach((coord, index) => {
            if (index > 0) {
                ctx.beginPath();
                ctx.moveTo(coordinates[0].x!, coord.y!);
    ...
    private drawLineValue(ctx: any, start: any, end: any, startIdx: number, endIdx: number) {
        ...
        const waveLabels = ['0-A', 'A-B', 'B-C', 'C-D', 'D-E'];
