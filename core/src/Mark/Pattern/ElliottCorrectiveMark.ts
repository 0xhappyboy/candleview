...
export class ElliottCorrectiveMark implements IGraph, IMarkStyle {
    private _chart: any;
    private _series: any;
    ...
    private markType: DrawingType = DrawingType.Elliott_Corrective;
        ...
        if (!this._renderer) {
            this._renderer = {
                draw: (target: any) => {
                    ...
                    this.drawLineValue(ctx, validCoordinates[0], validCoordinates[1], 'A-B');
                    this.drawLineValue(ctx, validCoordinates[1], validCoordinates[2], 'B-C');
                    this.drawLineValue(ctx, validCoordinates[2], validCoordinates[3], 'C-D');

                    
                    if (this._showHandles) {
                        const pointLabels = ['0', 'A', 'B', 'C'];
    ...
    private drawLineValue(ctx: any, start: any, end: any, waveLabel: string) {
        const midX = (start.x + end.x) / 2;
        const midY = (start.y + end.y) / 2;
        
        const startIndex = this._points.findIndex(p => 
            Math.abs(this._series.priceToCoordinate(p.price) - start.y) < 0.001 &&
            Math.abs(this._chart.timeScale().timeToCoordinate(p.time) - start.x) < 0.001
        );
        const endIndex = this._points.findIndex(p => 
            Math.abs(this._series.priceToCoordinate(p.price) - end.y) < 0.001 &&
            Math.abs(this._chart.timeScale().timeToCoordinate(p.time) - end.x) < 0.001
        );
