...
export class ElliottCorrectiveMarkManager implements IMarkManager<ElliottCorrectiveMark> {
  private props: ElliottCorrectiveMarkManagerProps;
  private state: ElliottCorrectiveMarkState;
  private previewMark: ElliottCorrectiveMark | null = null;
  private elliottCorrectiveMarks: ElliottCorrectiveMark[] = [];
  ...
  public getCurrentDragTarget(): ElliottCorrectiveMark | null {
  ...
  public getCurrentOperatingMark(): ElliottCorrectiveMark | null {
    ...
    if (this.state.isElliottCorrectiveMode && this.state.currentElliottCorrectiveMark) {
      return this.state.currentElliottCorrectiveMark;
    }
    return null;
  }

  public getAllMarks(): ElliottCorrectiveMark[] {
    return [...this.elliottCorrectiveMarks];
  }

  public cancelOperationMode() {
    return this.cancelElliottCorrectiveMode();
  }

  public setElliottCorrectiveMode = (): ElliottCorrectiveMarkState => {
    this.state = {
      ...this.state,
      isElliottCorrectiveMode: true,
      currentPoints: [],
      currentElliottCorrectiveMark: null,
      isDragging: false,
      dragTarget: null,
  ...
  public cancelElliottCorrectiveMode = (): ElliottCorrectiveMarkState => {
    if (this.previewMark) {
      this.props.chartSeries?.series.detachPrimitive(this.previewMark);
      this.previewMark = null;
    }
    this.elliottCorrectiveMarks.forEach(mark => {
  ...
  public handleMouseDown = (point: Point): ElliottCorrectiveMarkState => {
    const { chartSeries, chart, containerRef } = this.props;
    ...
    try {
      ...
      for (const mark of this.elliottCorrectiveMarks) {
        ...
        if (pointIndex !== null) {
          ...
          this.elliottCorrectiveMarks.forEach(m => {
      ...
      if (this.state.isElliottCorrectiveMode && !this.state.isDragging) {
        ...
        if (this.state.currentPoints.length === 0) {
          this.previewMark = new ElliottCorrectiveMark(newDataPoints, defaultColor);
          chartSeries.series.attachPrimitive(this.previewMark);
          this.elliottCorrectiveMarks.forEach(m => m.setShowHandles(false));
          this.previewMark.setShowHandles(true);
          this.state = {
            ...this.state,
            currentPoints: [point],
            currentElliottCorrectiveMark: this.previewMark
          };
          ...
          if (this.previewMark) {
            chartSeries.series.detachPrimitive(this.previewMark);
            const finalMark = new ElliottCorrectiveMark(newDataPoints, defaultColor);
            chartSeries.series.attachPrimitive(finalMark);
            this.elliottCorrectiveMarks.push(finalMark);
            this.previewMark = null;
  ...
  public handleMouseUp = (point: Point): ElliottCorrectiveMarkState => {
  ...
  public handleKeyDown = (event: KeyboardEvent): ElliottCorrectiveMarkState => {
    if (event.key === 'Escape') {
      if (this.state.isDragging) {
        if (this.state.dragTarget) {
          this.state.dragTarget.setDragging(false, null);
      ...
      } else if (this.state.isElliottCorrectiveMode) {
        return this.cancelElliottCorrectiveMode();
      }
    }
    return this.state;
  ...
  public getState(): ElliottCorrectiveMarkState {
    return { ...this.state };
  }

  public updateProps(newProps: Partial<ElliottCorrectiveMarkManagerProps>): void {
  ...
  public destroy(): void {
    ...
    this.elliottCorrectiveMarks = [];
    this.hiddenMarks = [];
  }

  public getElliottCorrectiveMarks(): ElliottCorrectiveMark[] {
    return [...this.elliottCorrectiveMarks];
  }

  public removeElliottCorrectiveMark(mark: ElliottCorrectiveMark): void {
    const index = this.elliottCorrectiveMarks.indexOf(mark);
    if (index > -1) {
      this.props.chartSeries?.series.detachPrimitive(mark);
      this.elliottCorrectiveMarks.splice(index, 1);
    }
  }

  public isOperatingOnChart(): boolean {
    return this.isOperating || this.state.isDragging || this.state.isElliottCorrectiveMode;
  }

  private hiddenMarks: ElliottCorrectiveMark[] = [];

  public hideAllMarks(): void {
    this.hiddenMarks.push(...this.elliottCorrectiveMarks);
    this.elliottCorrectiveMarks.forEach(mark => {
      this.props.chartSeries?.series.detachPrimitive(mark);
    });
    this.elliottCorrectiveMarks = [];
  ...
  public showAllMarks(): void {
    this.elliottCorrectiveMarks.push(...this.hiddenMarks);
  ...
  public hideMark(mark: ElliottCorrectiveMark): void {
    const index = this.elliottCorrectiveMarks.indexOf(mark);
    if (index > -1) {
      this.elliottCorrectiveMarks.splice(index, 1);
  ...
  public showMark(mark: ElliottCorrectiveMark): void {
    const index = this.hiddenMarks.indexOf(mark);
    if (index > -1) {
      this.hiddenMarks.splice(index, 1);
      this.elliottCorrectiveMarks.push(mark);
      this.props.chartSeries?.series.attachPrimitive(mark);
    }
  }
}